...
def resolve_http_transport_path() -> str | None:
    ...
    The secret travels in the X-API-Key HEADER, enforced by
    LocalTokenHeaderMiddleware, and the path is the plain base path.

    It used to ride in the URL path instead, on the assumption that header
    support was not guaranteed across the MCP clients this project drives. That
    assumption was measured on 2026-07-27 and is false: claude, kimi and codex
    all deliver a configured header on every MCP request. Meanwhile the URL form
    leaked the secret into workspace .mcp.json files the model can read, into
    `ps` output during client registration, and into log lines verbatim - four
    separate audit findings from one choice.

    The path form is NOT kept as a fallback. Keeping it would preserve exactly
    the leak this change exists to remove, and every config that carries it is
    regenerated by this project on each run. An old config now fails closed with
    401 rather than working while leaking.
    ...
    if not config.local_api_token:
        logger.error(
            ...
            "so the MCP endpoint on %s can authenticate: the listener is "
...
Examples:
  ...
  UNITY_MCP_TRANSPORT=http UNITY_MCP_HTTP_URL=http://localhost:9000 python -m src.server
    ...
    if config.transport_mode == 'http':
        ...
        # Redaction stays even though the secret no longer rides in the URL:
        # it also covers the header value and anything else that quotes it, and
        # it must be installed before the first request is served.
        if config.local_api_token:
            install_secret_redaction(config.local_api_token)
        # The transport carries every tool, including arbitrary C# execution in a
